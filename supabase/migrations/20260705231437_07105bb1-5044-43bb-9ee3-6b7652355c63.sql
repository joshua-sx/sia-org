...
AS $$ SELECT EXISTS (SELECT 1 FROM public.cycle_participants cp JOIN public.employees e ON e.id = cp.manager_id WHERE cp.id = p_participant_id AND e.profile_id = auth.uid()) $$;

CREATE OR REPLACE FUNCTION public.is_employee_of_participant(p_participant_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM public.cycle_participants cp JOIN public.employees e ON e.id = cp.employee_id WHERE cp.id = p_participant_id AND e.profile_id = auth.uid()) $$;

CREATE OR REPLACE FUNCTION public.is_extra_reviewer_of_participant(p_participant_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM public.cycle_participants cp JOIN public.employees e ON e.id = cp.extra_reviewer_id WHERE cp.id = p_participant_id AND e.profile_id = auth.uid()) $$;

CREATE OR REPLACE FUNCTION public.participant_final_submitted(p_participant_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT final_submitted_at IS NOT NULL FROM public.cycle_participants WHERE id = p_participant_id $$;

CREATE POLICY manager_read_participants ON public.cycle_participants
  FOR SELECT TO authenticated USING (public.is_manager_of_participant(id));
CREATE POLICY employee_read_own_participant ON public.cycle_participants
  FOR SELECT TO authenticated USING (public.is_employee_of_participant(id));
CREATE POLICY reviewer_read_participants ON public.cycle_participants
  FOR SELECT TO authenticated USING (public.is_extra_reviewer_of_participant(id));
...
CREATE POLICY employee_read_own_goals ON public.goals
  FOR SELECT TO authenticated USING (public.is_employee_of_participant(participant_id));
CREATE POLICY reviewer_read_goals ON public.goals
  FOR SELECT TO authenticated USING (public.is_extra_reviewer_of_participant(participant_id));
...
CREATE POLICY reviewer_read_ratings ON public.goal_ratings
  FOR SELECT TO authenticated USING (public.is_extra_reviewer_of_participant(public.goal_participant(goal_id)));
...
CREATE POLICY employee_read_own_ratings_after_final ON public.goal_ratings
  ...
  USING (public.is_employee_of_participant(public.goal_participant(goal_id))
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    v_mgr_change := NEW.rating IS DISTINCT FROM OLD.rating OR NEW.manager_comment IS DISTINCT FROM OLD.manager_comment;
...
BEGIN
  ...
  ELSE
    ...
    END IF;
  END IF;
  SELECT count(*), COALESCE(sum(weight), 0) INTO v_goal_count, v_weight_sum
  ...
  END IF;
  SELECT count(*) INTO v_unrated
  FROM public.goals g
  LEFT JOIN public.goal_ratings r ON r.goal_id = g.id AND r.stage = p_stage AND r.rating IS NOT NULL
