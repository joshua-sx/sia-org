-- Reject goal, rating, participant, and submit writes when the participant's
-- employee record is terminated. on_leave is treated like active.

CREATE OR REPLACE FUNCTION public.participant_employee_is_terminated(p_participant_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.participant_employee_is_terminated(uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  IF public.participant_employee_is_terminated(v_participant_id) THEN
...
BEGIN
  ...
  v_participant_id := public.goal_participant(v_goal_id);
  IF public.participant_employee_is_terminated(v_participant_id) THEN
    RAISE EXCEPTION 'SIA_EMPLOYEE_TERMINATED: this participant is no longer active in the organization';
  END IF;
...
REVOKE ALL ON FUNCTION public.guard_goal_writes() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.guard_rating_writes() FROM PUBLIC, anon, authenticated;
