...
-- Appraisal cycles: schema + baseline RLS
-- Tables: appraisal_cycles, cycle_participants, goals, goal_ratings.
-- Role policies, window/lock triggers and the submit RPC live in the
-- follow-up migration (20260705121000_appraisal_policies_rpc.sql).

-- 1. Helpers (SECURITY DEFINER so RLS policies can consult other tables
...
-- 2. Assessment stage weights on organizations
...
DO $$ BEGIN
  ALTER TABLE public.organizations
    ADD CONSTRAINT chk_org_stage_weights CHECK (interim_weight_pct + final_weight_pct = 100);
...
-- 4. Lookup helpers for child-table policies (bypass RLS, avoid recursion)
...
SET search_path = public
AS $$
  SELECT organization_id FROM public.appraisal_cycles WHERE id = p_cycle_id
$$;
...
SET search_path = public
AS $$
  SELECT participant_id FROM public.goals WHERE id = p_goal_id
$$;

-- 5. GRANTs (before RLS enable)
...
-- 6. RLS: enable + tenant isolation + hr_admin + org-wide cycle visibility.
-- Participant/goal/rating visibility for managers, employees and extra
-- reviewers is granted by role policies in the follow-up migration.
...
-- Cycles are visible to everyone in the org (windows drive all UI states).
...
-- Tenant isolation (RESTRICTIVE: ANDed with every permissive policy above/later)
