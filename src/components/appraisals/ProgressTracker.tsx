    ...
    <div className="rounded-xl border border-[hsl(var(--hairline))] bg-[hsl(var(--surface-raised))] shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
      ...
      >
        <span className="relative flex h-4 w-4 shrink-0 items-center justify-center">
          {complete ? (
            <span className="flex h-4 w-4 items-center justify-center rounded-full bg-[hsl(var(--accent-green))]">
            ...
            <>
              <span className="absolute inset-0 flex items-center justify-center opacity-100 transition-opacity group-hover:opacity-0">
                {open ? (
                  ...
                  <svg width="16" height="16" viewBox="0 0 16 16" className="shrink-0" aria-hidden="true">
                    <circle cx="8" cy="8" r={RING_R} fill="none" strokeWidth="2" stroke="hsl(var(--hairline))" />
                    <circle
                      cx="8"
                      cy="8"
                      r={RING_R}
              ...
              <span className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity group-hover:opacity-100">
                {open ? (
                  <ChevronDown size={14} className="text-[hsl(var(--ink-strong))]" />
                ) : (
                  <ChevronRight size={14} className="text-[hsl(var(--ink-strong))]" />
                )}
              </span>
            </>
          )}
        ...
        <span className="text-xs tabular-nums text-[hsl(var(--ink-subtle))]">
            ...
            {steps.map((step) => (
              ...
              >
                ...
                {step.status === "active" && step.sub && (
                  <span className="text-[11px] tabular-nums text-[hsl(var(--ink-muted))]">{step.sub}</span>
                )}
...
function StepDot({ status }: { status: TrackerStep["status"] }) {
  ...
  if (status === "active") {
    return (
      <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-[hsl(var(--ink-strong))]">
        <ArrowRight size={9} strokeWidth={2.5} className="text-[hsl(var(--ink-strong))]" />
      </span>
    );
  }
  return <span className="h-4 w-4 shrink-0 rounded-full border border-dashed border-[hsl(var(--ink-subtle))]" />;
}
...
function StepLabel({
  ...
  const className = [
    "text-[13.5px]",
    step.status === "done" && `pt-strike is-done ${animateStrike ? "" : "no-anim"} text-[hsl(var(--ink-subtle))]`,
    step.status === "active" && "font-medium text-foreground",
    step.status === "pending" && "text-[hsl(var(--ink-subtle))]",
  ]
    .filter(Boolean)
    .join(" ");
...
function ActionChip({ action }: { action: NonNullable<TrackerStep["action"]> }) {
  const className =
    "ml-auto shrink-0 rounded-md border border-[hsl(var(--hairline))] px-2 py-0.5 text-[11.5px] font-medium text-foreground transition-colors hover:border-[hsl(var(--ink-subtle))]";
