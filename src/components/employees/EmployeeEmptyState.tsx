      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {options.map((opt, i) => (
          <motion.button
            ...
            onClick={opt.onClick}
            className="group relative flex flex-col items-start gap-3 rounded-2xl border border-[hsl(var(--hairline))] bg-[hsl(var(--surface-raised))] p-5 text-left hover:border-[hsl(var(--ink-strong)/0.18)] hover:shadow-[0_4px_16px_-6px_rgba(0,0,0,0.08)] active:scale-[0.98]"
            style={{ transitionProperty: "border-color, box-shadow, transform", transitionDuration: "180ms" }}
          >
            {opt.badge && (
              <span
